...
sub new {
  ...
  my $self = bless {
    ...
    info_file           => $args{info_file}, # contains number of reads/errors/mutations generated by simCT
...
sub _init {
  ...
  if(defined $self->{info_file}) {
    ...
    my $info_it = CracTools::Utils::getFileIterator(file => $self->{info_file},
      parsing_method => \&CracTools::BenchCT::Utils::parseInfoLine,
    );
    while(my $info_line = $info_it->()) {
      if($info_line->{key} eq 'nb_reads') {
        $self->{nb_reads} = $info_line->{value};
      } elsif($info_line->{key} eq 'nb_errors') {
...
sub nbEvents {
  my $self = shift;
  my $type = shift;
  if($type eq 'mapping' && defined $self->{nb_reads}) {
    return $self->{nb_reads};
  } elsif($type eq 'error' && defined $self->{nb_errors}) {
    return $self->{nb_errors};
  } else {
...
sub isTrueError {
  ...
  # If we are using the "read names" encoded errors, we parse
  # the read_name to check alignments
  if(defined $self->{nb_errors}) {
    my $read = CracTools::BenchCT::Utils::parseReadName($read_name);
    return 1 if grep $pos == $_, @{$read->{errors}};
  # Otherwise we use the "error" events
  } else {
